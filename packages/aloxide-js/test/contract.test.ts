...
describe('Contract', () => {
  ...
  describe('Deploy eos contract to can testnet', () => {
    xit('Should deploy the contract ', async () => {
      ...
      const trn = await canTestnetService.deployContract({ wasmPath: EOS_TOKEN_WASM_PATH, abiPath: EOS_TOKEN_ABI_PATH}, new BlockchainAccount('aloxidejs123', '5JHQ3GuzcQtEQgG3SGvtDU7v2b7ioKznYBizA1V5mBUUsLNcXdQ'));
  ...
  describe('Deploy icon contract to icon testnet', () => {
    xit('Should deploy the contract ', async () => {
      ...
      const trn = await iconTestnetService.deployContract({ pyPath: ICON_TOKEN_PATH }, new BlockchainAccount('592eb276d534e2c41a2d9356c0ab262dc233d87e4dd71ce705ec130a8d27ff0c'), { params });
