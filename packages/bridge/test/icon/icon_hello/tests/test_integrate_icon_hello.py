...
class TestTest(IconIntegrateTestBase):
    ...
    def test_call_hi(self):
        # Generates a call instance using the CallBuilder
        call = CallBuilder().from_(self._test1.get_address())             .to(self._score_address)             .method("hi")             .build()

        # Sends the call request
        response = self.process_call(call, self.icon_service)

        self.assertEqual("Hi: " + str(self._test1.get_address()) , response)
